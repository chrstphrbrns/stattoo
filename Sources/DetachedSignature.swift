...
struct DetachedSignature {
	...
	func write(url:URL, encryptionKey:SecKey, hmacKey:Data) throws -> String? {
		...
		_ = writeExtendedAttribtue(s.data(using: .utf8)!.secureHash(withKey: hmacKey, algorithm: .sha256).hex().data(using: .utf8)!, named: "x-amz-meta-hmac", to: url.absoluteString)
		_ = writeExtendedAttribtue(keyid!.0.data(using: .utf8)!, named: "x-amz-meta-keytype", to: url.absoluteString)
		_ = writeExtendedAttribtue(keyid!.1.data(using: .utf8)!, named: "x-amz-meta-keyid", to: url.absoluteString)
	...
	static func getFromLocalCloud(for file:SignedFile) -> CloudResult? {
		if let url = file.cloudURL?.absoluteString {
			
			guard let hmac = readExtendedAttribute(named: "x-amz-meta-hmac", from: url) else {
				return nil
			}
			
			if let keytype = readExtendedAttribute(named: "x-amz-meta-keytype", from: url),
				let keyid = readExtendedAttribute(named: "x-amz-meta-keyid", from: url) {
				file.keyType = String(data: keytype, encoding: .utf8)!
				file.keyID = String(data: keyid, encoding: .utf8)!
				file.setKey(withKeyType: file.keyType!, keyID: file.keyID!)
			}
			...
			if let pem = try? Data(contentsOf: URL(fileURLWithPath: url)) {
				let hmac = String(data: hmac, encoding: .utf8)!
				if pem.secureHash(withKey: hmacKey, algorithm: .sha256).secureEquals(Data.fromHex(hmac)!) {
					if let data = extractPemPayloadData(pem) {
						if let encrytedData = EncryptedData.decode(data, withKey: hmacKey) {
							if let plain = encrytedData.decrypt(withKey: encryptionKey) {
								if let str = plain.string(using: .utf8) {
									if let timestamp = try? FileManager.default.attributesOfItem(atPath: url)[FileAttributeKey.modificationDate]! as? Date {
										if let sig = DetachedSignature(text: str, cloudTimestamp: timestamp) {
											return .signature(sig)
										}
									}
								}
